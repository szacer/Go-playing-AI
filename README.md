...
This is code used to create and train a Go-playing AI. Input features are created in C++ as opposed to Python for fast execution. A precompiled input feature is included (InputLabel.dll). Files to train the model (train.py) and play the model (play_model_vs_katago.py) are included. Additionally, the model weights are included as the files model_new_cross_ent_step=268200.h5 and model_new_cross_ent_step=268200.txt  
